...
有一小撮岗位，是机器人从**第三方招聘 API（Adzuna）**里刨出来的——这些公司我们还没直连它们
的官方招聘系统，属于「先搜来给你瞅瞅、回头再正式接入」的**过渡货**。所以：

- 表头会写着 **「Additional postings via Adzuna（第三方 API · 待接入）」**，看见这行就知道是这拨。
- 信息可能没那么精准（地点、职位名偶尔会糊），**一切以点进去的官网页面为准**。
- 但该看还得看——说不定金子就埋在这儿。等哪天正式接入了，它就搬到上面那张正表里去了。
