...
/**
 * Three-stage classification pipeline that assigns an SCM track
 * (ENTRY_LEVEL/INTERNSHIP/UNSURE/OTHER) to each job.
 *
 * <p>The stages run in order, each consuming only the jobs the previous stage couldn't classify:
 *
 * <ol>
 *   <li><b>Stage 1 — Title rules:</b> {@link JobTitleFilter#autoClassifyLevel} — INTERNSHIP /
 *       ENTRY_LEVEL from explicit title markers. Zero-cost.
 *   <li><b>Stage 2 — Description signals:</b> {@link SignalExtractor#inferLevelFromDescription} —
 *       enrollment → INTERNSHIP, YOE&gt;3 → OTHER, YOE 0–3 → ENTRY_LEVEL. Still local, no API call.
 *   <li><b>Stage 3 — Gemini LLM:</b> 4-way classification for the remaining ambiguous jobs.
 *       Batched, retried, most expensive.
 * </ol>
 *
 * <p>The caller receives a {@link Result} with the merged track map, the list of Gemini failures,
 * and per-stage counts for observability.
 */
@Slf4j
@Component
public class ClassificationPipeline {
     ...
     *
     * @param levelMap merged track assignments from all stages
